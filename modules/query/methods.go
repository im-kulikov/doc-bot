...
func (s *Service) Job(ctx context.Context) {
	...
	for i := 0; i < s.pool.Cap(); i++ {
		num := i
		if err := s.pool.Submit(s.worker(ctx, wg, num)); err != nil {
			s.log.Fatal("could start worker",
				zap.Int("num", num),
				zap.Error(err))
		}
	}
	...
	if err := s.pool.Release(); err != nil {
		s.log.Error("could not release pool",
			zap.Error(err))
	}
}

func (s *Service) worker(ctx context.Context, wg *sync.WaitGroup, num int) func() {
	return func() {
		log := s.log.With(
			zap.Int("worker", num))
